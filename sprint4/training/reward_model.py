"""Deterministic reward shaping for Sprint 4 Phase 2 policy training."""

from __future__ import annotations

from sprint4.training.benchmark_contract import is_repairable, is_unrecoverable
...
def is_abstain_action(action: PolicyAction) -> bool:
    """Return True when the policy explicitly abstains."""

    return action.action_type == "abstain"


def is_route_repair_action(action: PolicyAction) -> bool:
    """Return True when the action primarily repairs routing."""

    return action.action_type == "repair_route"


def is_auth_repair_action(action: PolicyAction) -> bool:
    """Return True when the action primarily repairs auth headers."""

    return action.action_type == "repair_auth"


def is_payload_repair_action(action: PolicyAction) -> bool:
    """Return True when the action primarily repairs payload structure."""

    return action.action_type == "repair_payload"


def is_action_safe_for_partition(
    action: PolicyAction,
    partition: str,
    scenario_type: str,
) -> bool:
    """Return whether the action stays inside basic safety bounds."""
...
def score_transition(
    ...
    """Score one transition using explicit, deterministic reward shaping."""
    ...
    if is_route_repair_action(action):
        reward.reward_route_accuracy += (
            1.0 if outcome.selected_route_correct else WRONG_ROUTE_PENALTY
        )

    if is_payload_repair_action(action):
        reward.reward_payload_accuracy += (
            1.0 if outcome.payload_valid else INVALID_PAYLOAD_PENALTY
        )
